...
# Check if Debian 12 or 13
DEBIAN_VERSION=$(cat /etc/debian_version | cut -d '.' -f 1)
if [ "$DEBIAN_VERSION" = "12" ] || [ "$DEBIAN_VERSION" = "13" ]; then
    echo "creating and activating virtual environment..."
    sudo -u "$OPENWB_USER" python3 -m venv "${OPENWBBASEDIR}/venv"
    source "${OPENWBBASEDIR}/venv/bin/activate"
fi
sudo -u "$OPENWB_USER" pip install -r "${OPENWBBASEDIR}/requirements.txt"
echo "installing openwb2 system service..."
# Modify openwb2.service to use virtual environment for Debian 12 or 13
ln -s "${OPENWBBASEDIR}/data/config/openwb2.service" /etc/systemd/system/openwb2.service
if [ "$DEBIAN_VERSION" = "12" ] || [ "$DEBIAN_VERSION" = "13" ]; then
    echo "updating openwb2.service to use virtual environment..."
    sed -i "s|ExecStart=/usr/bin/python3|ExecStart=${OPENWBBASEDIR}/venv/bin/python|" /etc/systemd/system/openwb2.service
fi
systemctl daemon-reload
